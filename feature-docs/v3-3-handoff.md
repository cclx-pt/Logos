...
| Decisão | Escolha |
|---|---|
| **Layout do CourseCard** | ~~Split 50/50 (imagem esquerda, texto direita).~~ **Revertido em 29-05-2026.** Mantém-se vertical (banner em cima, texto em baixo) em todos os tamanhos. Catálogo descarta descrição; /meus-cursos mantém. Cards crescem via `xl:grid-cols-4` na página full-width. |
| **Heading estado vazio /meus-cursos** | *"Inicia sessão para ver os teus cursos"* (assumida transcrição imperfeita de "Iniciação"). |
| **Modelo de enrollment** | **Reusar `course_access_log`**. Primeira row = enrolled. Adicionar coluna `unenrolled_at` na row mais recente (ou tabela auxiliar mínima) para permitir "sair do curso". Decisão fina (coluna vs tabela) na altura da PR8. |
...
| **RLS pública para anónimos** | A confirmar **na altura da PR8**. Direcção: relaxar SELECT de `courses` para `published_at IS NOT NULL AND required_tags = '{}'`. Não expõe nada novo — só permite ver sem login. |
| **Pesquisa em listas admin (PR6)** | Client-side filter sobre Server-rendered list, search input no topo de cada listagem (utilizadores, cursos, tags). Threshold para virar server-side fica para V4. |
| **Smoothness (PR7)** | Framer Motion **não** é introduzido. Usar transições CSS nativas + `<details>` HTML5 + `View Transitions API` quando possível. Mantém bundle leve. |
...
### Catálogo full-width (PR5)
Pedido em 29-05-2026 após teste de PR #39 (split 50/50): *"Quero que em desktop fique na horizontal na metade de cima e que a descrição seja só dentro do curso e não em conteúdos. (...) Gostaria também que em desktop a página ocupasse todo o ecrã."*

- **Layout do card mantém-se vertical** — banner na metade de cima, texto em baixo. Igual em mobile e desktop.
- **Descrição sai do catálogo** — vive só na landing do curso. Catálogo respira; títulos longos cabem; cards mais limpos.
- **/conteudos passa a full-width** em desktop (`w-full px-4 lg:px-12 xl:px-16`) — `xl:grid-cols-4` torna os cards maiores e aproveita o ecrã.
- /meus-cursos continua centrado (`max-w-5xl`) e mantém a descrição (cursos pessoais, contexto faz falta).

### CourseCard split 50/50 (PR4 — DESCARTADA)
~~Proposta de split horizontal foi implementada (PR #39) mas rejeitada após teste — sentiu-se "comprimida" e a descrição em catálogo congestionava. Revertida em PR5.~~
...
| ~~PR3~~ | Copy /meus-cursos (ícone fora + textos novos) | XS | Baixo | Não | ✅ mergeada |
| ~~PR4~~ | ~~CourseCard split 50/50~~ | M | Médio | Não | ❌ descartada (PR #39 closed) |
| **PR5** | Catálogo full-width + descrição só na landing do curso | S | Baixo | Não | — |
| **PR6** | Search em listagens admin + linhas clicáveis em `/admin/conteudos` | M | Baixo | Não | — |
| **PR7** | Smoothness pass — transitions, toast, view transitions, focus-visible | M | Médio (toca em muitos sítios) | Não | — |
| **PR8** | Enrollment + estado anónimo | L | Alto (schema + RLS + 3 views) | Sim — bloqueia merge final V3.3 | — |
...
- `v3-cursos → main` só depois de PR8 + testemunhos finais do ministério.
...
**PR5 — Catálogo full-width + sem descrição**
- `src/app/conteudos/conteudos-content.tsx` — `max-w-5xl` → `w-full px-4 lg:px-12 xl:px-16`; grid passa a `xl:grid-cols-4`
- `src/components/site/course-card.tsx` — descrição condicional, só fora de `catalog`
- Tests: `conteudos/page.test.tsx`

**PR6 — Search admin + linhas clicáveis**
