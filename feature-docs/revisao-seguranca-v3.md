...
- **Column-scoping de `profiles`**: já resolvido na V2.5 (`20260530160000`) - policy `update_own` removida, trigger torna `id`/`external_auth_id` imutáveis e guarda `role`. A policy `profiles_update_super_admin` que resta é column-guarded pelo mesmo trigger.
  > **Adenda (25-08-2026): esta migration resolveu o column-scoping e partiu outra coisa.** Ao recriar `enforce_profiles_role_mutation_authority()` por `create or replace`, a `20260530160000` partiu da versão que o **seu ramo** (`v2.5-copy-ux`) conhecia e reverteu, sem ninguém dar por isso, o alargamento a `super_admin` que a `20260530120000` (ramo `v3-cursos`, timestamp back-dated mais baixo) tinha feito. A promoção a super administrador ficou partida de 02-06 a 25-08; reposto pela `20260825120000`, que passa a manter as **duas** garantias na mesma definição. **Lição para revisões futuras:** endurecer por `create or replace` obriga a partir da última definição na ordem de aplicação, não da que o ramo tem à mão - e a verificar o que a versão antiga já permitia.
