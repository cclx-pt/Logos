...
> **Regressão e fix (25-08-2026):** entre 02-06 e 25-08 a promoção pela UI **esteve partida**. A migration que a permitiu (`20260530120000_allow_super_admin_promotion.sql`) nasceu no ramo `v3-cursos` com timestamp **12:00**, mas a `20260530160000_profiles_update_lockdown.sql` veio do ramo `v2.5-copy-ux` com timestamp **16:00** - ordena depois - e recriou a função a partir da versão de `20260514030344` (a única que existia no seu ramo), repondo `NEW.role not in ('user','admin')`. Resultado: o trigger recusava `super_admin` com errcode `22023`, e a página redirecionava para `?erro=generico` sem mostrar nada (o parâmetro nunca era lido), por isso a falha era silenciosa. Reposto pela `20260825120000_fix_super_admin_promotion_trigger.sql` (aplicada a BD live a 25-08-2026 e confirmada na UI), que passa a manter **uma única versão da função com as duas garantias juntas** (imutabilidade de `id`/`external_auth_id` + autoridade sobre `role` com `super_admin` válido). **Lição:** o trigger é tocado por `create or replace` em migrations de ramos diferentes; qualquer replace futuro tem de partir da última versão, não da que o ramo conhece - e timestamps de migration back-dated entre ramos escondem exactamente este tipo de perda.
