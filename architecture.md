...
| Fix da promoção a super administrador: repõe `super_admin` como valor válido no trigger de role, sem perder a imutabilidade de `id`/`external_auth_id` | `20260825120000` | ✅ aplicada no live (`dknrnqyqlojvnhspwjrd`) e no staging (`tirzriuabfwzqxtjsmfb`) |
...
- **Upload directo (V3.7, browser → Storage).** Igual às sebentas PDF (§7.1): o ficheiro **nunca passa por uma Server Action** (limite de ~4.5 MB de Functions na Vercel). O `CourseForm` pede uma signed upload URL (`createCourseBannerUploadUrlAction`, admin-only, `upsert`) e envia o ficheiro directamente para `<courseId>/banner`; create/update recebem só o `banner_storage_path` validado (`validateBannerStoragePath`). Tamanho (≤5 MB) e MIME impostos pelo bucket. **Wrinkle do create:** o path tem de levar o `courseId` no prefixo (fronteira da policy SELECT), mas no create o curso ainda não existe - por isso o id é **gerado no cliente** (`crypto.randomUUID`) e o curso é inserido com esse id explícito (`createCourseAction` aceita `id` opcional).
