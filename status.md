...
> **Última atualização:** 19-05-2026 (V3 PR2 schema base + storage aplicado a `logos-dev`; 73/73 testes verdes; preview Vercel actualizado)
...
## 🚧 Em progresso
- **V3 PR3 — Admin CRUD de Cursos** (próxima PR de V3). Detalhes em `feature-docs/v3-plan.md` §3: `/admin/cursos` listagem + `/admin/cursos/[id]` create/edit (title, slug, description, icon, required_tags multi-select alimentado por tags da PR1, published_at toggle), Server Actions com defesas, hard delete com AlertDialog de confirmação. 6-10 testes.
