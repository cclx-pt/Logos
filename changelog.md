...
### fix
- fix: [10-06-2026] **login Google rebentava com "This page couldn't load" (500)**. Carregar "Entrar" → Google dava erro de página em vez de ir para o Google. Causa: `signInWithGoogleAction` fazia `redirect()` para um URL **externo** (o do OAuth) dentro de uma Server Action - quando invocada pelo caminho **hidratado** (cliente, via `startTransition`/form com JS), o Next 16 rebenta com `Error: Connection closed.` (500). Reproduzido no preview: o caminho no-JS devolvia 303 (funcionava), o hidratado 500. Fix: a action passa a **devolver o URL** e o cliente navega com `window.location.assign` (padrão da Supabase para client components; os cookies de PKCE viajam na resposta da action). `signInWithGoogleAction(next?: string): Promise<string>`; `ProviderSignIn` deixa de ser `<form>`+`formAction` e passa a botões com `onClick` (o `next` vai em `data-next`); `SignInButton` (dropdown do cabeçalho) navega com o URL devolvido. Trade-off assumido: login passa a exigir JS. `getFirstLessonOfCourse`/redirect externo eram o único ponto frágil - os `redirect()` *internos* das outras actions (OTP, enroll, começar curso) não mudam. 4 testes adaptados (form/hidden-next → `data-next`) + 1 novo (clique chama a action e navega). 447 → 448 verdes.
