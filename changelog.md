...
### update
- update: [27-06-2026] **vídeos do tutorial arrancam em autoplay + chrome do YouTube reduzido** - o embed de `/como-funciona` passa a incluir `autoplay=1` (arranca ao mudar de passo, já que o iframe é remontado pela `key` do passo), `rel=0` + `modestbranding=1` (sem vídeos relacionados de outros canais nem logótipo) e `playsinline=1` (sem fullscreen forçado no iOS). Nota de browser: o autoplay com som só é permitido depois de uma interação, por isso o 1.º vídeo ao abrir a página pode precisar de um clique; os seguintes (após "Seguinte") arrancam sozinhos. O título do vídeo no topo é chrome do player que o YouTube já não deixa esconder por parâmetro (o antigo `showinfo` foi removido). Sem migration, sem env nova.
