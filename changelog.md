...
- infra: [25-08-2026] **previews deixaram de estar publicos e o staging foi posto em dia** - ao investigar o item pendente "aplicar migrations V3 ao staging" apareceu um problema maior. (1) **Deployment Protection do projeto Vercel estava toda desligada** (`ssoProtection`, `passwordProtection`, `trustedIps` a `false`, confirmado pela API), ao contrario do que o `status.md` afirmava, **e** os previews apontavam para a base de dados de **producao** - o item "apontar o Preview scope" do runbook nunca foi feito, e o "deixar como esta" ficou a apontar para o `dknrnqyqlojvnhspwjrd`, que virou producao no lancamento. Verificado no bundle: preview e `logos.cclx.pt` serviam ambos o mesmo ref. Resultado: cada URL de preview era uma app publica, sem login, com codigo por rever a escrever em dados reais. **Sem segredo exposto** - a publishable key e publica por desenho e a RLS e a mesma do site normal; o risco era o codigo, nao as credenciais. Corrigido ligando **Vercel Authentication** em `all_except_custom_domains` (protege previews e aliases `.vercel.app`, mantem `logos.cclx.pt` publico); preview passou de 200 a 302 e producao mantem 200. (2) **Staging (`tirzriuabfwzqxtjsmfb`) despausado e migrado**: levou as **19 migrations V3** que lhe faltavam - nunca tinha recebido nenhuma, porque o lancamento foi troca de env e nao `db push`. Aplicadas por `supabase db push --include-all` depois de `migration list` (ground truth do ledger) e `--dry-run` confirmarem o plano; `--include-all` foi preciso porque `20260530120000` e `20260530130000` ordenam **antes** de migrations ja aplicadas em 02-06. Verificado: 0 por aplicar, schema V3 completo por `inspect db`, e o trigger de role na versao merged (`super_admin` aceite **e** imutabilidade de `external_auth_id`) - a `20260530120000` sozinha teria deixado o staging sem o segundo. (3) **`.env.example`**: o comentario dizia "em Vercel Production usa-se logos-prod", ao contrario da realidade desde o lancamento, no ficheiro que se le exactamente ao configurar um ambiente; reescrito a avisar que os nomes enganam e a guiar pelo ref. Removida a `SUPABASE_STORAGE_BUCKET`, que nao e lida em lado nenhum e so nomeava um dos dois buckets. **Parado aqui por decisao:** o Preview scope nao foi mudado e a auth do staging nao foi configurada - nenhuma variavel de ambiente foi tocada (verificado por `vercel env ls`), logo nao ha nada a meio. Com a proteccao dos previews ligada o risco fechou, e o que resta e higiene que so compensa feita de uma assentada: apontados para o staging sem OTP, os previews ficam inutilizaveis (o Google nao funciona em URLs de preview, logo nao haveria como entrar). Levantamento das variaveis da Vercel, metodo de separacao (painel, nao CLI) e raciocinio registados em `feature-docs/launch-runbook.md`.
- infra: [25-08-2026] **`next` 16.2.6 -> 16.2.12 (4 advisories HIGH) + `pnpm.overrides` refrescados** (continua o varrimento de 24-08). O `next@16.2.6` acumulava quatro HIGH, todos corrigidos em `16.2.11` e todos na superfície que esta app usa a sério: bypass de middleware/proxy no App Router (Turbopack + locale único), DoS no App Router via Server Actions, SSRF em Server Actions com servidor customizado, e SSRF em rewrites por hostname de destino controlado pelo atacante - com a app assente em Server Actions (`courses`/`lessons`/`modules-actions`), o risco não era teórico. Bump para **16.2.12**, o último patch da linha 16.2 (mesma minor, zero mudança de API); `eslint-config-next` acompanha. **Overrides refrescados** - cada pin fora escrito para um advisory mais antigo e já não cobria o atual: `postcss` `>=8.5.10` -> `>=8.5.18` (path traversal no auto-load de sourcemap), `brace-expansion` `>=5.0.6` -> `>=5.0.9` (DoS), `hono` `>=4.12.21` -> `>=4.12.34`. O pin do `brace-expansion` é **deliberadamente limitado** a `>=4.0.0 <5.0.9`: um override abrangente (`brace-expansion: >=5.0.9`) empurra a linha 5.x para dentro do `minimatch@3.1.5`, que espera a API 1.x, e rebenta o eslint com `TypeError: expand is not a function`; com o selector limitado a árvore fica com **1.1.18 e 5.0.9**, ambas as linhas corrigidas e o eslint a funcionar. `pnpm audit` de **53 para 27** advisories (25 HIGH para 12); os que ficam resolvem todos por tooling de dev (`jsdom`/`vitest`, `eslint`, CLI `shadcn`) e nunca chegam a produção. **Fora de propósito:** `sharp@0.34.5` (HIGH herdado do libvips, corrigido em `0.35.0`) - o `next` 16.2 fixa `sharp: ^0.34.5` como optional dep, por isso a única via limpa é o `next` 16.3.x, que já pede `^0.35.3`. Esse salto foi **tentado e reprovado**: o 16.3 removeu `experimental.viewTransition` do schema de config (typecheck a falhar com `TS2353`), que é a flag que liga o **crossfade entre rotas da V3.3 PR7** - apagá-la sem mais deixaria a feature cair em silêncio. Fica como decisão própria, com smoke ao crossfade. Sem migration, sem env nova. 697 testes + `next build` (38 rotas) verdes com o lockfile novo.

### fix
- fix: [25-08-2026] **as tabelas do admin cortavam colunas em mobile - e não havia sequer scroll** - reportado como "não se vê tudo no admin em telemóvel". As cinco tabelas da área admin (`/admin/utilizadores`, `/admin/estatisticas` e sub-páginas, `/admin/conteudos`, `/admin/etiquetas`) viviam dentro de um contentor `overflow-hidden`. Abaixo de ~1000px a largura mínima da tabela passa a do ecrã, e o `overflow-hidden` **cortava as colunas da direita sem deixar deslizar** - ficavam invisíveis e inalcançáveis, não apenas apertadas. Em `/admin/utilizadores` (7 colunas) o que desaparecia era justamente a coluna de acções ("Promover a admin" / "Promover a super admin"); em `/admin/estatisticas` (7 colunas) metade dos números. **Fix:** novo `TableScroll` (`src/components/admin/table-scroll.tsx`) substitui os cinco contentores por `overflow-x-auto` - a correcção é **só CSS**, funciona sem JavaScript. Por cima, melhoria progressiva depois da hidratação: quando (e só quando) há mesmo o que deslizar, o contentor ganha `role="region"` + `tabIndex` (scroll pelo teclado, WCAG 2.1.1) e aparece a dica "Desliza a tabela para o lado para veres todas as colunas."; um `ResizeObserver` vigia o contentor **e** a tabela, porque a largura muda ao rodar o telemóvel e também quando o `ListSearch` esconde linhas. Cabeçalhos, datas e papéis passam a `whitespace-nowrap` (deixam de partir em colunas de uma sílaba); as colunas de texto da `SortableStatsTable` continuam a partir linha, senão um título de módulo longo fazia a tabela crescer sem fim. **De caminho:** os 4 cartões de total em `/admin/estatisticas/cursos/[courseId]` estavam a `grid-cols-2` em mobile (dois por linha a ~165px, com a dica de cada um a partir em 4 linhas) - passam a um por linha até `sm`, como já era em `/admin/estatisticas`; e o comentário do `ConteudosBreadcrumb` dizia que a nav do admin é `hidden md:block`, o que deixou de ser verdade no fix de 25-06. **Varrimento ao resto da interface em mobile** não encontrou mais nada cortado: a nav do admin já colapsa em chips, a `CourseTree` é `hidden xl:block` por desenho (o breadcrumb cobre a navegação abaixo de xl), a página de aula já tem versões mobile próprias (caixa de perguntas, dica da sebenta) e as páginas públicas usam `flex-wrap` + grelhas responsivas. Sem migration, sem env nova. **703 testes** (3 novos do `TableScroll`) + lint + typecheck + `next build` verdes.
- fix: [25-08-2026] **promover um utilizador a super administrador nao funcionava (regressao de merge entre ramos) + a pagina nao dava qualquer feedback** - reportado apos tentativa falhada em `/admin/utilizadores`. **Causa:** a migration `20260530120000_allow_super_admin_promotion.sql` alargou o trigger `enforce_profiles_role_mutation_authority` para aceitar `super_admin` em `NEW.role`, mas foi escrita no ramo `v3-cursos` (merge 28-06) com timestamp **12:00**. A migration `20260530160000_profiles_update_lockdown.sql` veio do ramo `v2.5-copy-ux` (merge 02-06) com timestamp **16:00** - ou seja, **ordena depois** - e fez `create or replace function` a partir da unica versao que existia no seu ramo (a de `20260514030344`), acrescentando a imutabilidade de `id`/`external_auth_id` mas **repondo `NEW.role not in ('user','admin')`** (o proprio comentario diz "inalterado de 20260514030344"). Na base de dados vive a versao 160000, logo qualquer promocao a super administrador rebentava no trigger com `Papel invalido via UI (recebido: super_admin)` (errcode 22023) - apesar de a UI e o `setUserRoleAction` a permitirem. **Fix:** nova migration `20260825120000_fix_super_admin_promotion_trigger.sql` com **uma unica versao da funcao que junta as duas garantias** (imutabilidade de `id`/`external_auth_id` a partir de contexto autenticado **e** role mutavel so por super_admin, nunca sobre um super_admin existente, com `super_admin` entre os valores validos), para nao voltar a perder-se nenhuma num futuro replace; mantem o `revoke execute` (o `create or replace` devolve o grant default). Democao de super_admin continua so-SQL, por desenho. **Segundo bug, o que escondeu o primeiro:** as actions de `/admin/utilizadores` redirecionam com `?guardado=` / `?erro=`, mas a pagina **nunca lia esses parametros** - sucesso e falha eram visualmente identicos (nada acontecia no ecra). Adicionado banner de feedback (`role="alert"` / `role="status"`) com as mensagens de `papel_atualizado`, `etiquetas_atribuidas`, `selecao_vazia` e `generico`. **Aplicada à BD live** (ref `dknrnqyqlojvnhspwjrd`) a 25-08-2026 pelo SQL Editor do dashboard, com a versao do ficheiro registada a mao em `supabase_migrations.schema_migrations` (o MCP do Supabase falha o OAuth com `Unrecognized client_id`; um `db push` arriscava mexer no resto do ledger, ja divergente). Verificado na BD e na UI - a promocao funciona. O antigo `logos-prod` (`tirzriuabfwzqxtjsmfb`, hoje dev/staging) mantem o trigger antigo ate um `db push` futuro. 697 testes + lint + typecheck verdes. Sem env nova.
- fix: [24-08-2026] **em dashes em copy visível ao utilizador (regra dura PT-PT)** - varrimento pós-lançamento encontrou 6 ocorrências de `—` em texto que o utilizador lê, todas anteriores ao lançamento: `/conteudos/[courseId]` ("Em breve —" nos dois ramos sem aulas publicadas e "Parabéns — concluíste todas as aulas deste curso"), `/admin/conteudos/[courseId]` (subtítulo "Despublicar não apaga —") e `/admin/conteudos/novo` ("ajustar tudo mais tarde —", passou a dois pontos por introduzir uma enumeração). No `IconPicker`, o glifo "sem ícone" era um `—` solto em `text-lg`: passou ao ícone `Ban` do lucide a `h-5 w-5`, igual às outras opções da grelha (mantém `aria-hidden` + `sr-only "Sem ícone"`). Restantes ocorrências de `—` na árvore são comentários de código, fora do âmbito da regra. Sem migration, sem env nova.
- fix: [24-08-2026] **`src/lib/courses/overview-stats.ts` deixa de ser invisível ao `grep`** - o ficheiro usa um separador NUL para compor a chave `${user_id}<NUL>${course_id}` do mapa de acessos, mas o NUL estava gravado como **byte cru** no código-fonte. Isso fazia o `git`/`file`/`grep` classificarem o ficheiro como binário, e o `grep` **saltava-o em silêncio** - o ficheiro escapou ao varrimento de em dashes acima e escaparia a qualquer auditoria futura de PT-PT/copy. O byte cru passa à sequência de escape `\u0000` (mesmo valor em runtime, zero mudança de comportamento) e o ficheiro volta a ser UTF-8 legível por ferramentas de texto. Corrigido de caminho um comentário desatualizado: dizia que o card "Utilizadores registados" mostra "—" quando a RPC falha, mas `admin/estatisticas/page.tsx` renderiza `n/d`.

### docs
- docs: [24-08-2026] **`feature-docs/pre-lancamento-handoff.md` apagado** - o handoff descrevia o lançamento V3 como pendente ("PR única no dia do lançamento", "migrations → `logos-prod`", "repetir config externa em `logos-prod`"), mas a V3 lançou a 28-06-2026 (PR #70) e `main` é produção desde então. Pior: apontava o `logos-prod` como alvo do lançamento, quando o ref que ficou **live** é o `dknrnqyqlojvnhspwjrd` (o antigo `logos-dev`, promovido por troca de env) - ler o handoff hoje leva à BD errada numa operação destrutiva. Regra do `CLAUDE.md`: apagar o handoff quando o trabalho fecha, já que os registos definitivos vivem em `feature-docs/`, `changelog.md` e `status.md`. Referência pendente em `status.md` removida.
...
### update
- update: [28-06-2026] **terminologia: "apostila" -> "sebenta" em toda a UI/copy/docs** (PT-PT - "sebenta" é o termo de Portugal; "apostila" lê mais PT-BR; alinha com a regra dura de PT-PT na UI). Renomeadas as 81 ocorrências em 26 ficheiros, com preservação de caso e plural (Apostila/apostila/Apostilas -> Sebenta/sebenta/Sebentas): strings visíveis (botão "Descarregar sebenta", heading "Sebenta", hero "Cursos, Sebentas e o teu ritmo", metadados de `/conteudos`, labels do form de aula), comentários de código e docs (SPEC, architecture, status, feature-docs). Testes atualizados em lockstep com as strings (`page.test.tsx`, `pdf-download-button.test.tsx`); os 18 testes afetados ficam verdes. O `id`/`aria-labelledby` interno do leitor passou a `sebenta-heading`. **Excluídas de propósito as migrations já aplicadas** (`20260519020000`, `20260612120000`): editar uma migration aplicada só mexe em comentários, não propaga ao `COMMENT` da BD em produção e cria divergência ficheiro<->BD - o COMMENT vivo dessas colunas mantém "apostila" (interno; sem efeito na UI). Sem migration, sem env nova.
...
### fix
- fix: [30-06-2026] **sebentas em mobile: download deixa de falhar em silêncio + visualizador deixa de dar "sem permissão" intermitente** (2 bugs reportados no telemóvel). (1) **Download:** o botão "Descarregar sebenta" fazia `window.open(url)` **depois** de um `await` à Server Action (`getLessonPdfSignedUrlAction`). Em mobile (sobretudo iOS Safari) o bloqueador de popups só deixa abrir nova janela durante o **gesto síncrono** do toque; perdido esse gesto após o await, o popup era bloqueado **em silêncio** e nada acontecia (no desktop o bloqueador é permissivo, daí parecer só-mobile). (2) **Visualizador:** a página embebia no `<iframe>` uma signed URL de **5 min** assinada no render; em mobile os browsers descarregam/recarregam iframes fora de vista, e ao recarregar após 5 min a URL estava expirada → o Supabase devolvia o seu erro de acesso **dentro do iframe** (lia como "sem permissão"); intermitente por depender do tempo/rede. **Solução comum:** novo route handler `GET /conteudos/[courseId]/[lessonId]/sebenta` que **assina fresco a cada pedido** (núcleo extraído para `src/lib/courses/lesson-pdf.ts`, partilhado com a Server Action mantida por compat). O botão passa a `<a href=".../sebenta?dl=1">` (navegação real, sem `window.open`; `?dl=1` força `Content-Disposition: attachment`) e o iframe aponta para `.../sebenta` (inline) - nunca mais uma URL expirada no HTML, e a signed URL deixa de aparecer no DOM. Em mobile, dica "Se a sebenta não abrir aqui, usa o botão Descarregar sebenta" (muitos browsers não renderizam PDF em iframe). CSP: `frame-src` ganha `'self'` (o iframe aponta para a rota same-origin que faz 302 para o bucket; a CSP valida cada salto). **Importante: não é o sistema de etiquetas** - conteúdo restrito por etiqueta é invisível, nunca aparece com erro. Sem migration, sem env nova. 697 verdes; lint + typecheck limpos. Docs: `feature-docs/lesson-pdf-upload.md`.
...
- feat: [12-06-2026] **modelo de aula só-vídeo (`video`)** (V3.4 PR1) - uma aula passa a poder ser `pdf` (só sebenta), `video` (só vídeo do YouTube) ou `video_pdf` (ambos). DB: migration `20260612120000_lessons_video_only_template.sql` relaxa os CHECK de `lessons` (sebenta deixa de ser sempre obrigatória; vídeo passa a exigir `youtube_url`; só-vídeo guarda `pdf_storage_path = null`) - **só `logos-dev`**. Server Actions (`createLessonAction`/`updateLessonAction`) validam YouTube quando o template tem vídeo e PDF quando tem sebenta; trocar para `video` limpa o path e remove o ficheiro do bucket best-effort. **Form de aula condicional** (item 2): novo Client Component `lesson-form.tsx` mostra só os campos do template escolhido (só PDF → ficheiro; só vídeo → URL; ambos → os dois). Leitor de aula renderiza vídeo/sebenta conforme o template; etiquetas dos 3 modelos no admin (`só pdf`/`só vídeo`/`vídeo + pdf`) e no leitor (`pdf`/`vídeo`/`vídeo + pdf`, via `template-label.ts`). SPEC bump 3.2; tipos `LessonTemplate` + `pdf_storage_path` nullable em `detail.ts`. 461 verdes.
...
- fix: [10-06-2026] **CSP bloqueava o Supabase Storage - banners e sebentas PDF "desapareciam"**. Regressão silenciosa do port de hardening (#47): a paridade foi verificada contra `main`, mas `main` (V2) não tem banners de cursos nem visualizador inline de sebentas. `img-src` não permitia as signed URLs dos banners (`course-banners`, renderizados por `<CourseImage>` com `unoptimized` no catálogo, `/meus-cursos`, landing do curso e preview do admin) e `frame-src` só permitia YouTube + Turnstile, bloqueando o iframe da sebenta (`lesson-pdfs`) na página de aula. `next.config.ts` ganha `https://*.supabase.co` nas duas directivas (wildcard cobre `logos-dev` e `logos-prod` sem acoplar a CSP a env vars - mesmo racional do `connect-src`). Teste de regressão novo `src/test/security-headers.test.ts` pina as origens externas de cada directiva + o conjunto completo de headers. 440 → 445 verdes.
...
- `src/components/site/home-hero.tsx` — subtítulo do hero passa de "O ministério LOGOS é o espaço da CCLX..." para **"Cursos, Sebentas e o teu ritmo — Sempre gratuitos."**
...
- add: `src/app/conteudos/[slug]/[lessonId]/page.tsx` — breadcrumb 3 níveis (Conteúdos › Curso › Aula); módulo + título + descrição; iframe YouTube em `aspect-video` rounded-2xl com `loading="lazy"`, `youtube-nocookie.com` (privacidade), `allow` standard, `allowFullScreen`; botão "Descarregar sebenta" (Client); navegação anterior/próxima atravessando fronteira de módulo; "Voltar ao curso" se for a última; índice do curso colapsável (módulo actual `open` por defeito) com indicador visual `aria-current="page"` na aula activa.
...
- add: migration `supabase/migrations/20260519020000_v3_courses_schema_and_storage.sql` aplicada a `logos-dev`.
  ...
  - **`lessons`**: module_id CASCADE, position, title, description, template CHECK in ('pdf','video_pdf'), youtube_url nullable, pdf_storage_path **not** nullable (V3 exige sebenta), CHECK `video_pdf → youtube_url IS NOT NULL`. Índice composto `(module_id, position)`.
...
- update: `src/app/cursos/cursos-content.tsx` — passa de placeholder simples a layout com intro + secção "O que vais encontrar" (3 cards: vídeo embebido, sebenta PDF, ritmo próprio — icons `BookOpen`, `FileDown`, `CheckCircle2`). Mantém tag "Em breve".
...
- add: `docs/branding/mockups-v3.jpeg` — quatro mockups V3 (catálogo, aula, módulo, sebenta) — referência vinculativa de paleta e estrutura
