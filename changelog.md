...
### update
- update: [27-06-2026] **vídeos do tutorial em autoplay mudo + loop + chrome do YouTube reduzido** - o embed de `/como-funciona` passa a incluir `autoplay=1` + `mute=1` (arranca sozinho em qualquer browser - o autoplay só é permitido sem som - logo no 1.º vídeo e a cada mudança de passo, já que o iframe é remontado pela `key` do passo), `loop=1` + `playlist=<id>` (repete o vídeo no fim; o loop do YouTube para um vídeo único exige o `playlist` com o mesmo id), `rel=0` + `modestbranding=1` (sem vídeos relacionados de outros canais nem logótipo) e `playsinline=1` (sem fullscreen forçado no iOS). Decisão do líder: os vídeos não precisam de som, por isso ficam mudos para garantir o autostart. O título do vídeo no topo é chrome do player que o YouTube já não deixa esconder por parâmetro (o antigo `showinfo` foi removido). Sem migration, sem env nova.
